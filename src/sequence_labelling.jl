...
function BiLSTM_CNN_CRF_Model(labels, chars_idx, words_idx, UNK_char_idx,UNK_Word_idx, weights_path)
    ...
    conv1 = load(joinpath(weights_path, "conv_cpu.bson"))[:conv_cpu]
    W_word_Embed = load(joinpath(weights_path, "W_word_cpu.bson"))[:W_word_cpu].data
    W_Char_Embed = load(joinpath(weights_path, "W_char_cpu.bson"))[:W_char_cpu].data
    forward_lstm = load(joinpath(weights_path, "forward_lstm.bson"))[:forward_lstm_cpu]
    backward = load(joinpath(weights_path, "backward_lstm.bson"))[:backward_lstm_cpu]
    d_out = load(joinpath(weights_path, "d_cpu.bson"))[:d_cpu]
    c = load(joinpath(weights_path, "crf.bson"))[:crf_cpu]
